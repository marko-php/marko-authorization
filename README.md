...
Authorization provides ability-based access control. Define abilities with closures via the Gate, or organize them into policy classes mapped to entities. Use `#[Can]` on controller methods to enforce permissions automatically via middleware. Denials throw `AuthorizationException` with clear context.
...
The `AuthorizationMiddleware` reads `#[Can]` attributes and checks the Gate automatically.

### Implementing AuthorizableInterface

Your user entity must implement `AuthorizableInterface`:
